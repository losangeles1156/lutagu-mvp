...
function loadStations() {
  return JSON.parse(fs.readFileSync(STATIONS_PATH, 'utf8')) as Array<{ id: string; name: string; lat: number; lng: number }>;
}

function assignStations(landmark: Landmark, stations: ReturnType<typeof loadStations>) {
...
async function main() {
  ...
  const landmarks: Landmark[] = JSON.parse(fs.readFileSync(path.resolve(process.cwd(), inputPath), 'utf8'));
