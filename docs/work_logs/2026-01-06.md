...
### 困難 4：前端 IntlError（缺少翻譯鍵）與 L2 狀態請求中止

**問題描述**：
- 在 `ja/en` 語系下切換節點或開啟對話時，出現 `IntlError: MISSING_MESSAGE`，導致部分 UI/對話區崩潰或互動失效
- 對話開啟後會觸發 `/api/l2/status` 請求，瀏覽器中出現 `net::ERR_ABORTED` 造成誤判為系統錯誤

**根本原因**：
1. 翻譯檔缺少關鍵鍵值（例如：`ja.common.retry`、`en.l1.noHighlights`）
2. L2 狀態 fetch 未設計 timeout/abort cleanup，造成頁面切換或狀態更新時請求被中止

**解決方式**：
1. 補齊翻譯鍵值：
   - `messages/en.json`: 新增 `l1.noHighlights`
   - `messages/ja.json`: 新增 `common.retry`
2. 增加 i18n 缺鍵防護：
   - `src/i18n.ts`: 啟用 `onError` 與 `getMessageFallback`，避免缺鍵直接拋錯造成 UI 崩潰
3. 修正 L2 狀態請求：
   - `src/components/chat/ChatOverlay.tsx`: 增加 `AbortController` + 15s timeout，並在 effect cleanup 中 abort
4. 新增回歸測試：
   - `src/lib/i18n/messages.test.ts`: 驗證關鍵翻譯鍵存在，避免再次回歸
