...
import type { RequestContext } from '../HybridEngine'; // Using type import to avoid circular dependency runtime issues
    ...
    async dispatch(input: string, context: RequestContext): Promise<{ skill: DeepResearchSkill, score: number } | null> {
        ...
        for (const skill of skills) {
            ...
            if (skill.calculateRelevance) {
                score = await skill.calculateRelevance(input, context);
            } else {
