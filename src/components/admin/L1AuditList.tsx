            ...
            <div className="flex justify-between items-center mb-6">
                <div className="flex gap-4 items-center">
                    <div className="relative">
                        ...
                        <select
                            ...
                            className="pl-9 pr-4 py-2 border border-slate-200 rounded-xl text-sm appearance-none bg-white min-w-[180px] focus:ring-2 focus:ring-slate-900/20 focus:border-slate-400"
                        >
                    ...
                    <div className="flex bg-slate-100 rounded-xl p-1">
                        <button
                            ...
                            className={`px-3 py-1.5 text-xs font-medium rounded-lg transition ${statusFilter === 'pending' ? 'bg-white shadow text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                            待審核
                        </button>
                        <button
                            onClick={() => setStatusFilter('approved')}
                            className={`px-3 py-1.5 text-xs font-medium rounded-lg transition ${statusFilter === 'approved' ? 'bg-white shadow text-emerald-700' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                            已發布
                        </button>
                        <button
                            onClick={() => setStatusFilter('all')}
                            className={`px-3 py-1.5 text-xs font-medium rounded-lg transition ${statusFilter === 'all' ? 'bg-white shadow text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                            全部
                        </button>
                    </div>
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        className="px-4 py-2 bg-slate-900 text-white rounded-xl flex items-center gap-2 text-sm font-medium hover:bg-slate-800 transition-colors"
                    >
                        <Plus size={16} /> 新增 POI
                    </button>
                </div>
                ...
                <table className="admin-table">
                    <thead>
                        <tr className="bg-slate-50 border-b border-slate-200">
                            <th className="px-6 py-3 text-left">名稱 / ID</th>
                            <th className="px-6 py-3 text-left">分類</th>
                            <th className="px-6 py-3 text-left">短評 (Review)</th>
                    ...
                    <tbody className="bg-white divide-y divide-slate-100">
                        {loading ? (
                            <tr><td colSpan={5} className="py-10 text-center text-slate-400">載入中...</td></tr>
                        ) : places.length === 0 ? (
                            <tr><td colSpan={5} className="py-10 text-center text-slate-400">無資料</td></tr>
                        ) : places.map(place => (
                            <tr key={place.id} className="hover:bg-gray-50 group transition-colors">
                                <td className="px-6 py-4">
                                    <div className="font-semibold text-slate-900">{place.name_i18n.ja || place.name_i18n.en}</div>
                                    <div className="text-xs text-slate-400 flex items-center gap-1 mt-0.5">
                                        <span className="truncate max-w-[150px]">{place.station_id}</span>
                                        {place.affiliate_url && <ExternalLink size={10} className="text-slate-400" />}
                                    </div>
                                </td>
                                <td className="px-6 py-4">
