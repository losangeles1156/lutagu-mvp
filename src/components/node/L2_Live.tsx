...
export function L2_Live({ data, hubDetails }: L2_LiveProps) {
    ...
    const { lines, crowd, updatedAt, weather, explanations } = (data.l2 || {
        lines: [],
            ...
            <div className="space-y-2">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        {weather?.alertLevel === 'watch' && delayedLines.length === 0 && (
                            <span className="px-2 py-0.5 bg-amber-50 text-amber-700 text-[9px] font-bold rounded-full flex items-center gap-1">
                                <AlertTriangle size={10} /> {tL2('weatherWatchOnly', { defaultValue: 'Weather watch (operation normal)' })}
                            </span>
                        )}
                    </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden p-1">
                    {explanations && (
                        <div className="mx-2 my-2 rounded-xl border border-indigo-100 bg-indigo-50 px-3 py-2">
                            <p className="text-[11px] text-indigo-900 font-semibold">
                                {locale.startsWith('en') ? explanations.plain_en : explanations.plain_zh_tw}
                            </p>
                            {explanations.official_ja && (
                                <p className="text-[10px] text-indigo-600 mt-1 line-clamp-2">原文: {explanations.official_ja}</p>
                            )}
                        </div>
                    )}
                    {lines.length === 0 ? (
